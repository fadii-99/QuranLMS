...
@section('content')
    ...
    <div id="edit-teacher-modal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50">
        <div class="bg-white dark:bg-gray-700 rounded-2xl p-6 w-full max-w-md backdrop-blur-lg shadow-xl relative">
            ...
            <form action="{{ route('admin.teacher.update') }}" method="POST" class="space-y-4">
                @csrf
                ...
                <div class="relative">
                    <label for="edit-password" class="block text-sm text-gray-600 dark:text-gray-300 mb-1">Password</label>
                    <input id="edit-password" name="password" type="password" autocomplete="off"
                        class="w-full pr-10 px-4 py-2 rounded-lg bg-gray-100 dark:bg-gray-600 text-gray-800 dark:text-gray-200 border border-gray-300/50 dark:border-gray-600/50 focus:ring-2 focus:ring-primary transition">
                    <button type="button" id="toggleEditPassword"
                        class="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-500 dark:text-gray-400">
                        <i class="fas fa-eye absolute right-4 top-2/3 -translate-y-1/2" id="editEyeIcon"></i>
                    </button>
                    <script>
                        // Password toggle for edit form
                        const editPwdInput = document.getElementById('edit-password');
                        ...
                        toggleEditBtn.addEventListener('click', () => {
                            const type = editPwdInput.getAttribute('type') === 'password' ? 'text' : 'password';
                            editPwdInput.setAttribute('type', type);
                            editEyeIcon.classList.toggle('fa-eye');
    ...
    <div id="assign-subject-modal"
        ...
        <div class="bg-white dark:bg-gray-700 rounded-2xl p-6 w-full max-w-md shadow-xl relative">
            ...
            <div id="current-subjects-list" class="mb-4 hidden">
                <p class="mb-2 text-gray-600 dark:text-gray-300">Current Subjects:</p>
                <div id="assigned-subjects-container" class="flex flex-wrap gap-2"></div>
            </div>
        ...
        <table class="min-w-full">
            <thead>
                <tr class="bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300">
                    ...
                    <th class="px-4 py-3 text-left">No. Students</th>
                    <th class="px-4 py-3 text-left">No. Subjects</th>
                    ...
                    <tr class="border-t border-gray-200 dark:border-gray-600">
                        ...
                        <td class="px-4 py-2 flex space-x-2">
                            <button class="assign-subject text-green-500 hover:text-green-700"
                                ...
                                data-assigned-subjects='@json($teacher->subjects->map(fn($s) => ['id' => $s->id, 'name' => $s->name]))'>
                    ...
                    <tr>
                        <td colspan="7" class="px-8 py-12 text-center">
                            <div class="flex flex-col items-center justify-center space-y-3">
                                ...
                                <p class="text-gray-400 dark:text-gray-500 text-sm">Click on "New Teacher" button to add a
                                    teacher</p>
                            </div>
                        </td>
                    </tr>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            confirmDeleteBtn.addEventListener('click', async () => {
                const teacherId = confirmDeleteBtn.dataset.teacherId;
                try {
                    const response = await fetch(`/admin/teacher/delete`, {
                        method: 'POST',
                        headers: {
                    ...
                    showToast('error', 'Could not delete teacher. Please try again.');
                }
            ...
            assignButtons.forEach(btn => {
                btn.addEventListener('click', () => {
                    ...
                    const assignedSubjects = JSON.parse(btn.dataset.assignedSubjects || "[]");

                    teacherNameEl.textContent = teacherName;

                    // Show all current subjects in modal
                    const currentSubjectsList = document.getElementById('current-subjects-list');
                    const assignedSubjectsContainer = document.getElementById(
                        'assigned-subjects-container');
                    assignedSubjectsContainer.innerHTML = '';

                    if (assignedSubjects.length) {
                        currentSubjectsList.classList.remove('hidden');
                        assignedSubjects.forEach(subject => {
                            assignedSubjectsContainer.innerHTML += `
                                <span class="flex items-center px-2 py-1 rounded bg-blue-100 text-blue-800 text-xs mr-2 mb-2">
                                    ${subject.name}
                                    <button class="remove-assigned-subject ml-1 text-red-500" data-teacher-id="${teacherId}" data-subject-id="${subject.id}" title="Remove">
                        ...
                        currentSubjectsList.classList.add('hidden');
                    }
                ...
                btn.addEventListener('click', async () => {
                    ...
                    try {
                        const res = await fetch(
                            '{{ route('admin.teacher.assign-subject') }}', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
                        ...
                        showToast('error', 'Could not assign subject. Try again.');
                    }
                });
            });

            // Remove assigned subject (cross)
            document.addEventListener('click', async function(e) {
                if (e.target.closest('.remove-assigned-subject')) {
                    const btn = e.target.closest('.remove-assigned-subject');
                    const teacherId = btn.dataset.teacherId;
                    const subjectId = btn.dataset.subjectId;
                    try {
