...
@section('content')
    ...
    function completeRequest(id) {
        fetch('{{ url("superadmin/request") }}/' + id + '/complete', {
            method: 'POST',
            headers: {
    ...
    document.getElementById('cancel-modal-confirm').onclick = function(){
        fetch('{{ url("superadmin/request") }}/' + cancelId + '/cancel', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Accept': 'application/json',
    ...
    function viewRequest(id){
        fetch('{{ url("superadmin/request") }}/' + id + '/view', {
            method: 'GET',
                    ...
                    <div class="space-y-6 p-6 bg-gray-50 dark:bg-gray-700 rounded-lg max-h-[70vh] overflow-y-auto">
                        <div class="flex flex-col md:flex-row md:items-center gap-4">
                            ...
                            <span class="text-gray-800 dark:text-gray-100 flex items-center flex-wrap gap-2">
                                ${item.user.name} 
                                <span class="px-3 py-1 text-sm bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 rounded-full">${item.user.academy_name}</span>
                            </span>
                        </div>
                        <div class="flex flex-col md:flex-row md:items-center gap-4">
